...
class EmailService {
    ...
    async sendEmailViaBrevo(to, subject, htmlContent, attachments) {
        if (!this.apiKey) {
            throw new ApiError_1.ApiError('Brevo API key is not configured.', 500);
        }
    ...
    async sendWelcomeEmail(email, firstName, resetToken) {
    ...
    async sendMissionOrderEmail(email, employeeName, missionNumber, pdfBuffer) {
        const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
    ...
    `;
        ...
        await this.sendEmailViaBrevo(email, `MAS - Mission Order / Ordre de mission - ${missionNumber}`, htmlContent, attachments);
    }
}
